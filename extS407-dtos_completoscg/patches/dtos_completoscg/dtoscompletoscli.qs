...
function interna_init()
{
  ...
  switch (cursor.modeAccess()) {
    case cursor.Insert: {
      ...
      this.iface.bloqueoCliente_ = true;
      this.iface.comprobarVacio("codcliente", "fdbNombre");
      this.iface.comprobarVacio("codgrupo", "fdbNombreGrupo");
      ...
      this.iface.bloqueoArticulo_ = false;
      this.iface.bloqueoCliente_ = false;
      break;
    }
    case cursor.Edit: {
      this.iface.bloqueoArticulo_ = true;
  ...
  if (vacio) {
    ...
    switch (campoCodigo) {
      ...
      case "codcliente": {
        ...
        this.child("fdbCodGrupo").setDisabled(false);
      break;  
	  }
	  case "codgrupo": {
        this.child("fdbCodCliente").setDisabled(false);
    ...
    switch (campoCodigo) {
	  case "codcliente": {
        this.child("fdbCodGrupo").setDisabled(true);
        this.child("fdbCodGrupo").setValue("");
        this.child("fdbNombreGrupo").setValue(todos);
      break;  
	  } 	
	  case "codgrupo": {
        this.child("fdbCodCliente").setDisabled(true);
        this.child("fdbCodCliente").setValue("");
        this.child("fdbNombre").setValue(todos);
      break;  
	  } 	
      case "referencia": {
...
function oficial_comprobarUnicidad(): String {
  ...
  criterios["codgrupo"] = cursor.valueBuffer("codgrupo");
...
function oficial_dameWhere(criterios: Array): String {
  ...
  if (criterios["codgrupo"] != "")
  {
    where += " AND codgrupo = '" + criterios["codgrupo"] + "'";
  } else {
    where += " AND codgrupo IS NULL";
  }
...
function oficial_dameIntervaloDto(criterios: Array, fecha: String): Array {
  ...
  var query: FLSqlQuery = new FLSqlQuery();
  query.setTablesList("dtoscompletoscli");
  query.setSelect("coddescuento, dtopor, dtolineal, sumar, codcliente, codgrupo, referencia, codsubfamilia, codfamilia");
  query.setFrom("dtoscompletoscli");
  query.setWhere(where + whereFecha + " ORDER BY codcliente, codgrupo, referencia, codsubfamilia, codfamilia");
  query.setForwardOnly(true);
...
function oficial_dameWhereDto(criterios: Array): String {
  ...
  if (criterios["codgrupo"] != "")
  {
    where += " AND (codgrupo = '" + criterios["codgrupo"] + "' OR codgrupo IS NULL)";
  } else {
    where += " AND codgrupo IS NULL";
  }
