	...
	function busqLineasDocs_cambioleBusqueda()
{
		...
		if (this.iface.leBusqueda.text != '') {
			...
			{
				this.iface.tdbRecords.cursor().setMainFilter(this.iface.filtroOriginal + " AND idpresupuesto in (select idpresupuesto from lineaspresupuestoscli where upper(referencia) like upper('%" + busquedaTxt + "%') or upper(descripcion) like upper('%" + busquedaTxt + "%'))");
			}
			else
			{
