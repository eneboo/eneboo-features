...
//debug(">>>>>>> A buscar a cadena seguinte: " + busquedaTxt);
		...
		if (this.iface.leBusqueda.text != '') {
			...
			{
				this.iface.tdbRecords.cursor().setMainFilter(this.iface.filtroOriginal + " AND idfactura in (select idfactura from lineasfacturasprov where upper(referencia) like upper('%" + busquedaTxt + "%') or upper(descripcion) like upper('%" + busquedaTxt + "%'))");
