...
function orderCols_init()
{
	...
	connect(this.child("tdbLineasAlbaranesProv").cursor(), "newBuffer()", this, "iface.reordenarColumnas");

	var util: FLUtil = new FLUtil;
	var cursor:FLSqlCursor = this.child("tdbLineasAlbaranesProv").cursor();
	...
	if (anchos.length == orden.length) { 
	    for (var i = 0; i < anchos.length; i++) {
		  this.child("tdbLineasAlbaranesProv").setColumnWidth(orden[i], anchos[i]);
	    }
	} else {
	...
	this.child("tdbLineasAlbaranesProv").switchSortOrder(formRecordordercols.iface.pub_ascendenteDescendente(cursor));
