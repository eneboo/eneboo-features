...
{
	this.iface.__init();
 connect(this.child("tdbRegStocks").cursor(), "newBuffer()", this, "iface.reordenarColumnas");
  ...
  if (anchos.length == orden.length) { 
      ...
      this.child("tdbRegStocks").setColumnWidth(orden[i], anchos[i]);
    }
	} else {
 ...
 this.child("tdbRegStocks").switchSortOrder(formRecordordercols.iface.pub_ascendenteDescendente(cursor));
...
function orderCols_reordenarColumnas()
{
  /* Deixamos os anchos das columnas soamente */
	...
	var util: FLUtil = new FLUtil;
	var cursor:FLSqlCursor = this.child("tdbRegStocks").cursor();
	var orden:Array = formRecordordercols.iface.pub_aplicarOrden(cursor);
	var anchos:Array = formRecordordercols.iface.pub_aplicarAnchos(cursor);
	if (anchos.length == orden.length) {
	    for (var i = 0; i < anchos.length; i++) {
