...
function orderCols_init()
{
	...
	var cursor:FLSqlCursor = this.child("tdbArticulosPedProv").cursor();
	...
	if (anchos.length == orden.length) { 
	    for (var i = 0; i < anchos.length; i++) {
		  this.child("tdbArticulosPedProv").setColumnWidth(orden[i], anchos[i]);
	    }
	} else {
	...
	this.child("tdbArticulosPedProv").switchSortOrder(formRecordordercols.iface.pub_ascendenteDescendente(cursor));
