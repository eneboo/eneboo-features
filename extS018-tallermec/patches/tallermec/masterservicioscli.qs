...
class ifaceCtx extends head {
	...
	function pub_imprimir(numServicio:String) {
			return this.sol_imprimir(numServicio);
	}
}
class sol extends ifaceCtx{
    var tbnImprimirS:Object;
    function sol( context ) { ifaceCtx( context );}
    function imprimir(numServicio:String) {
		return this.ctx.sol_imprimir(numServicio);	    }
    function imprimirS(numServicio:String) {
	return this.ctx.sol_imprimirS(numServicio); }
   function init() { this.ctx.sol_init(); }
}

const iface = new sol( this );
//// INTERFACE  /////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
...
function interna_init()
{
	...
	//SIAGAL this.iface.tbnImprimir = this.child("toolButtonPrint");
	...
	// SIAGAL connect(this.iface.tbnImprimir, "clicked()", this, "iface.imprimir");
...
function oficial_datosAlbaran(cursor:FLSqlCursor,curAlbaran:FLSqlCursor,where:String):Boolean
{
	...
	var codvehiculo:String = cursor.valueBuffer("codvehiculo");
	var matricula:String = cursor.valueBuffer("matricula");
	var cambioac:Number = cursor.valueBuffer("cambioac");
	var kilometros:Number = cursor.valueBuffer("kilometros");
	var combustible:String = cursor.valueBuffer("combustible");
	var totalRepuestos:Number = cursor.valueBuffer("totalrepuestos");
	var totalMano:Number = cursor.valueBuffer("totalmano");
	var totalServicio:Number = cursor.valueBuffer("totalservicio");
	...
	// SIAGAL - Pasamos el servicio al albarán
	var numServicio:String = cursor.valueBuffer("numservicio");
	...
	with(curAlbaran) {	
		...
		setValueBuffer("codvehiculo", codvehiculo);
		setValueBuffer("matricula", matricula);
		setValueBuffer("cambioac", cambioac);
		setValueBuffer("kilometros", kilometros);
		setValueBuffer("combustible", combustible);
		setValueBuffer("totalrepuestos", totalRepuestos);
		setValueBuffer("totalmano", totalMano);
		setValueBuffer("totalalbaran", totalServicio);
		setValueBuffer("manod", cursor.valueBuffer("manod"));
		setValueBuffer("manoc", cursor.valueBuffer("manoc"));
		setValueBuffer("manop", cursor.valueBuffer("manop"));
		setValueBuffer("manoi", cursor.valueBuffer("manoi"));
		setValueBuffer("descl", cursor.valueBuffer("descl"));
		setValueBuffer("descp", cursor.valueBuffer("descp"));
		...
		setValueBuffer("numservicio", numServicio);
...
function oficial_datosLineaServicio(curLineaServicio:FLSqlCursor,curLineaAlbaran:FLSqlCursor,idAlbaran:Number):Boolean
{
	...
	with(curLineaAlbaran) {
		...
		setValueBuffer("proximocambio", curLineaServicio.valueBuffer("proximocambio"));
		setValueBuffer("codfamilia", curLineaServicio.valueBuffer("codfamilia"));
	}
	return true;
}
...
function sol_imprimir(numServicio:String)
{
	...
	idServicio = util.sqlSelect("servicioscli", "idservicio", "numservicio = '" + this.cursor().valueBuffer("numservicio") + "'");
	var curLineaServicio:FLSqlCursor = new FLSqlCursor("lineasservicioscli");
	var curLineaObra:FLSqlCursor = new FLSqlCursor("lineasobra");
if (!curLineaObra.select("idservicio = "+ idServicio + " OR idservicio <> "+ idServicio ))
    return false;
...
if(!curLineaServicio.select("idservicio = " + idServicio + " AND (codfamilia<> 'MO' OR codfamilia is null)"))
    return false;
	while (curLineaServicio.next()) {
	    curLineaServicio.setModeAccess(curLineaServicio.Browse);
		curLineaServicio.refreshBuffer();
	    ...
	    curLineaObra.refreshBuffer();
	    with(curLineaObra) {
		setValueBuffer("idlinea", curLineaServicio.valueBuffer("idlinea"));
		setValueBuffer("idservicio", curLineaServicio.valueBuffer("idservicio"));
		setValueBuffer("referencia", curLineaServicio.valueBuffer("referencia"));
		setValueBuffer("descripcion", curLineaServicio.valueBuffer("descripcion"));
		setValueBuffer("pvpunitario", curLineaServicio.valueBuffer("pvpunitario"));
		setValueBuffer("cantidad", curLineaServicio.valueBuffer("cantidad"));
		setValueBuffer("pvpsindto", curLineaServicio.valueBuffer("pvpsindto"));
		setValueBuffer("pvptotal", curLineaServicio.valueBuffer("pvptotal"));
		setValueBuffer("codimpuesto", curLineaServicio.valueBuffer("codimpuesto"));
		setValueBuffer("iva", curLineaServicio.valueBuffer("iva"));
		...
		setValueBuffer("dtopor", curLineaServicio.valueBuffer("dtopor"));
		setValueBuffer("irpf", curLineaServicio.valueBuffer("irpf"));
		setValueBuffer("kilometros", curLineaServicio.valueBuffer("kilometros"));
		setValueBuffer("proximocambio", curLineaServicio.valueBuffer("proximocambio"));
		//setValueBuffer("cod", curLineaServicio.valueBuffer("kilometros"));
		setValueBuffer("codfamilia", curLineaServicio.valueBuffer("codfamilia"));
		if(!curLineaObra.commitBuffer())
		    return false;
	}
...
function sol_imprimirS(numServicio:String)
{
		if (sys.isLoadedModule("flfactinfo")) {
				...
				curImprimir.setValueBuffer("d_servicioscli_numservicio", numero);
				curImprimir.setValueBuffer("h_servicioscli_numservicio", numero);
				flfactinfo.iface.pub_lanzarInforme(curImprimir, "vld_iservicioscli");
		} else
				flfactppal.iface.pub_msgNoDisponible("Informes");
...
function sol_init()
{
      ...
      this.iface.tbnImprimirS = this.child("toolButtonPrintS");
      connect(this.iface.tbnImprimir, "clicked()", this, "iface.imprimir");
      connect(this.iface.tbnImprimirS, "clicked()", this, "iface.imprimirS");
}
