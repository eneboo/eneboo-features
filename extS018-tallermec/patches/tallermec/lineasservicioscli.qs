...
function interna_init()
{
		...
		case cursor.Edit:{
			  cantvieja=cursor.valueBuffer("cantidad");
			  break;
			}
		}
...
function interna_acceptedForm()
{
    ...
    codarticulo = cursor.valueBuffer("referencia");
    ...
    var curStock:FLSqlCursor = new FLSqlCursor("stocks");
        if(!curStock.select("referencia = '" + codarticulo + "'"))
		return false;
	...
	cant = curStock.valueBuffer("cantidad");
	debug("interna_acceptedForm cant: " + cant);
	cant = cant - this.child("fdbCantidad").value();
	debug("interna_acceptedForm cant = cant - fdbCantidad: " + cant);
	cant = cant + cantvieja;
	debug("interna_acceptedForm cant = cant - cantvieja" + cant);
	
	
	
	
	
	curStock.setValueBuffer("cantidad",cant);
	curStock.setValueBuffer("disponible",cant);
	curStock.commitBuffer();
      if(!curArticulo.select("referencia = '" + codarticulo + "'"))
	  return false;
      curArticulo.first();
      smin = curArticulo.valueBuffer("stockmin");
      if(cant==smin || cant<smin)
	  MessageBox.information("El stock de este producto está bajo mínimo, stock disponible = "+cant);
...
//}
 ...
 //	formRecordlineaspedidoscli.iface.pub_commonBufferChanged(fN, form);
	...
	case "kilometros": {
			var cursor:FLSqlCursor = this.cursor();
			cursor.setValueBuffer("proximocambio", this.iface.calculateField("proximocambio"));
			break;
...
function oficial_calculateField(fN:String):String
{
	//return formRecordlineaspedidoscli.iface.pub_commonCalculateField(fN, this.cursor());
 	...
 	switch (fN) {
		...
		case "proximocambio": {
			  if (cursor.valueBuffer("kilometros")!=""){
				
				valor = parseFloat(cursor.valueBuffer("kilometros")) + parseFloat(cursor.cursorRelation().valueBuffer("kilometros"));
				if (!valor) 
					valor = "";
			} else {
				valor = "";
