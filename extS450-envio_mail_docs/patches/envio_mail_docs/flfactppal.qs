...
/////////////////////////////////////////////////////////////////
//// ENVIO MAIL /////////////////////////////////////////////////
class envioMail extends numerosLinea /** %from: oficial */
{
  ...
  function extension(nE)
  {
    ...
    function marcarEnviadoMail(idDocumento: String, campoIdDocumento:String, tablaDocumento:String)
  {
    return this.ctx.envioMail_marcarEnviadoMail(idDocumento,campoIdDocumento,tablaDocumento);
  }
}
//// ENVIO MAIL /////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////
//// PUB ENVIO MAIL /////////////////////////////////////////////
class pubEnvioMail extends sepa19b2b /** %from: envioMail */
{
  ...
  function pub_marcarEnviadoMail(idDocumento: String, campoIdDocumento:String, tablaDocumento:String)
  {
    return this.marcarEnviadoMail(idDocumento,campoIdDocumento,tablaDocumento);
  }
}
...
function envioMail_marcarEnviadoMail(idDocumento: String, campoIdDocumento:String, tablaDocumento:String) {


  //debug(">>>>>>>>>>>>>>>>> flfactppal envioMail_marcarEnviadoMail idDocumento:" + idDocumento + " - campoIdDocumento:" + campoIdDocumento + " - tablaDocumento:" + tablaDocumento);

  var documento: String = tablaDocumento;
  var nombreIdDocumento: String = campoIdDocumento;
  var util: FLUtil = new FLUtil();
  var bloqueado: Boolean = false;
  var cursor: FLSqlCursor = new FLSqlCursor(documento);

  cursor.select(nombreIdDocumento + " = " + idDocumento);
  cursor.first();
  cursor.setActivatedCommitActions(false);
  cursor.setActivatedCheckIntegrity(false);

  if (cursor.valueBuffer("enviadomail")) {
	MessageBox.warning(util.translate("scripts", "Este documento ya fue enviado por mail por el usuario %1.\n\nSerá enviado de nuevo y marcado envío con usuario y fecha actual.").arg(cursor.valueBuffer("enviadomailidusuario")), MessageBox.Ok, MessageBox.NoButton);
  }


  // O Campo de bloqueo nos albaráns de cliente é outro
  var campoBloqueo:String = "editable";
  
  if (tablaDocumento == "albaranescli")
	campoBloqueo = "ptefactura";
	
	


  if (cursor.valueBuffer(campoBloqueo) == false)
  {
    cursor.select(nombreIdDocumento + " = " + idDocumento);
    cursor.first();
    cursor.setUnLock(campoBloqueo, true);
    bloqueado = true;
  }

  var hoxe:Date = new Date()
  var hora:String = hoxe.toString().mid(11,8);
  cursor.select(nombreIdDocumento + " = " + idDocumento);
  cursor.first();
  cursor.setModeAccess(cursor.Edit);
  cursor.refreshBuffer();
  cursor.setValueBuffer("enviadomail", true);
  cursor.setValueBuffer("enviadomailfecha", hoxe);
  cursor.setValueBuffer("enviadomailhora", hora);
  cursor.setValueBuffer("enviadomailidusuario", sys.nameUser());

  if (!cursor.commitBuffer())
  {
    MessageBox.warning(util.translate("scripts", "Error marcando el documento como enviado email"), MessageBox.Ok, MessageBox.NoButton);
  }

  // Se estaba bloqueado o devolvemos ao estado bloqueado
  if (bloqueado)
  {
    cursor.select(nombreIdDocumento + " = " + idDocumento);
    cursor.first();
    cursor.setUnLock(campoBloqueo, false);
  }
