...
function interna_init()
{
  ...
  connect(cursor, "bufferChanged(QString)", this, "iface.bufferChanged");
   ...
   connect(this.child("tdbLineasPresupuestosProv").cursor(), "bufferCommited()", this, "iface.calcularTotales()");

  connect(this.child("tbnSubir"), "clicked()", this, "iface.tbnSubir");
  connect(this.child("tbnBajar"), "clicked()", this, "iface.tbnBajar");

  this.child("tdbLineasPresupuestosProv").putFirstCol("numlinea");
  ...
  //CONTROLAMOS OS STOCKS POR LINEA
  connect(this.child("tdbLineasPresupuestosProv").cursor(), "newBuffer()", this, "iface.filtrarDatos()");
  this.iface.filtrarDatos();


  //Se está xetsionada pois pechamos
  if (cursor.modeAccess() == cursor.Edit) {
		if (cursor.valueBuffer("estado") == "Tramitada")
		{
			MessageBox.warning(util.translate("scripts", "La gestión de compra ya fue tramitada\ny pasada a un pedido.\n\nContacte con un administrador si es necesario."), MessageBox.Ok, MessageBox.NoButton);
			this.child("tbwPresupuesto").setTabEnabled("contacto", false);
			this.child("tbwPresupuesto").setTabEnabled("observaciones", false);
			this.child("tbwPresupuesto").setTabEnabled("datos", false);
			this.child("tbwPresupuestosProv").setTabEnabled("precios", false);
			this.child("tbwPresupuestosProv").setTabEnabled("lineas", false);

			this.child("fdbIdPresupuesto").setDisabled(true);
			this.child("fdbCodPresupuesto").setDisabled(true);
			//this.form.close();
		}
  }
  ...
  switch (fN) {
    ...
    case "estado": {
      ...
      if (cursor.valueBuffer("estado") == "Aprobada") {
        ...
        cursor.setValueBuffer("frecepcion",hoy);
      }
      break;
    }
  }
...
{
  var cursor: FLSqlCursor = this.cursor();
  var referencia: String = this.child("tdbLineasPresupuestosProv").cursor().valueBuffer("referencia");

  //SIAGAL se non hai barcode pasamos de filtrar por el
  var andBarcode: String = "";
  var barcode: String = this.child("tdbLineasPresupuestosProv").cursor().valueBuffer("barcode");
  if (barcode)
...
function oficial_tbnSubir()
{
  ...
  var cursor = this.child("tdbLineasPresupuestosProv").cursor();
  var row = this.child("tdbLineasPresupuestosProv").currentRow();
  ...
  this.child("tdbLineasPresupuestosProv").refresh();
  row += -1;
  this.child("tdbLineasPresupuestosProv").setCurrentRow(row);
}
...
function oficial_calcularTotales()
{
  ...
  valor = util.sqlSelect("lineaspresupuestosprov", "SUM((pvpunitario - (pvpunitario * dto /100)) * cantidad)", "idpresupuesto = " + cursor.valueBuffer("idpresupuesto"));
  
  debug(">>>SIAGAL presupuestosprov.qs   oficial_calcularTotales   valor:" + valor);
  valor = parseFloat(AQUtil.roundFieldValue(valor, "pedidosprov", "neto"));
  this.child("fdbNeto").setValue(valor);
...
/* Sobrecargamos a oficial que Ã© a que estÃ¡ conectada co tdbArticulosPedProv  */
