...
class busqEnDocs extends ofertasProv /** %from: oficial */
{
  ...
  var filtroNuevoDoc:String;
	...
	function busqEnDocs( context ) { ofertasProv ( context ); }
	function init() {
...
function busqEnDocs_init()
{
	...
	/** \D Filtra los documentos por lciente tendo en conta o filtro das liñas
\end */
function busqEnDocs_cambioleBusquedaDoc()
{
  ...
  if (this.iface.leBusquedaDoc.text != "") {
      this.iface.filtroOriginalDoc = this.iface.tdbRecords.cursor().mainFilter();
      filtroDoc = " AND pedidoscli.nombrecliente like upper('%" + busquedaTxt + "%')";
